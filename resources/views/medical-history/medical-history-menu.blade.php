        ...
        <section class="text-gray-600 body-font">
            <div class="container px-5 py-24 mx-auto">
                                    ...
                                    @else
                                        ...
                                        <form id="delete_{{ $medicalHistory->id }}" method="post" action="{{ route('user.medical-history.destroy', ['medical_history_id' => $medicalHistory->id]) }}">
                                            ...
                                            <a href="#" data-id="{{ $medicalHistory->id }}" onclick="deleteMedicalHistory(this)">
                                                <button type="button" class="flex items-center mt-auto mb-1 text-white bg-red-400 border-0 py-2 px-4 w-full focus:outline-none hover:bg-red-500 rounded">削除
                                                    <svg fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" class="w-4 h-4 ml-auto" viewBox="0 0 24 24">
                                                    <path d="M5 12h14M12 5l7 7-7 7"></path>
                                                    </svg>
                                                </button>
                ...
                @if(!empty($medicalHistories))
                    <div class="mx-auto mt-5">{{ $medicalHistories->links() }}</div>
                @endif
            </div>
            <!-- マイページボタン -->
